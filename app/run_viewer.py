#!/usr/bin/env python3
"""
マルチモーダルデータセットビューア起動スクリプト (Poetry対応)
"""
...
def run_viewer():
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(current_dir)
    viewer_path = os.path.join(current_dir, "dataset_viewer.py")
    
    if not os.path.exists(viewer_path):
        print(f"エラー: {viewer_path} が見つかりません。")
        return
    
    print("マルチモーダルデータセットビューアを起動しています...")
    ...
    try:
        ...
        subprocess.run([
            "poetry", "run", "streamlit", "run", viewer_path,
            "--server.port", "8501",
        ...
        print(f"ビューアの起動に失敗しました: {e}")

def main():
    print("=== マルチモーダルデータセットビューア (Poetry版) ===")
